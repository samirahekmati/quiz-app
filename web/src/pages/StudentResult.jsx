	...
	useEffect(() => {
		async function fetchResult() {
			...
			try {
				...
				const res = await fetch(`${getApiBaseUrl()}/quizzes/${quizId}/result`, {
					credentials: "include",
				});
	...
	return (
		<div className="p-4 max-w-md mx-auto text-center">
			...
			{loading ? (
				...
				<div className="border p-4 rounded bg-gray-50 text-gray-600 text-center">
					<p className="mb-2 font-semibold">Username: {result.username}</p>
					<p className="mb-2">Quiz ID: {result.quizId}</p>
					<p className="mb-2">Correct Answers: {result.correctAnswers}</p>
					<p className="mb-2">Incorrect Answers: {result.incorrectAnswers}</p>
					<p className="mb-2">Total Answered: {result.total}</p>
				</div>
			) : (
				<div className="border p-4 rounded bg-gray-50 text-gray-600 text-center">
