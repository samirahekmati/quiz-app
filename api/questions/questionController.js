...
export async function createQuestion(req, res) {
	...
	try {
		...
		res
			.status(500)
			.json({ message: "Server error while creating question and options" });
	} finally {
		client.release();
	}
}
...
export async function updateQuestion(req, res) {
	...
	if (!questionId || isNaN(Number(questionId))) {
		return res
			.status(400)
			.json({ message: "Valid questionId is required in the URL." });
	}
	...
	try {
		...
		if (checkRes.rowCount === 0) {
			...
			return res
				.status(404)
				.json({ message: "Question not found for this quiz." });
		}
		...
		if (optionsToDelete.length > 0) {
			await client.query(`DELETE FROM options WHERE id = ANY($1)`, [
				optionsToDelete,
			]);
		}
		...
		res
			.status(500)
			.json({ message: "Server error while updating question and options." });
	} finally {
		client.release();
	}
}
...
export async function deleteQuestion(req, res) {
	...
	try {
		...
		// Delete related options first (unless you have ON DELETE CASCADE set)
